-- 013_goat_cut_sheets.sql
-- Goat cut sheet tables, validation trigger, and seed defaults (whole + half).
-- Run: psql -d terra_mensa -f sql/013_goat_cut_sheets.sql

-- ─── Tables ──────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS goat_cut_sheets (
...
CREATE TABLE IF NOT EXISTS goat_cut_sheet_defaults (
    id              SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_goat_cut_sheets_po ON goat_cut_sheets(po_number);

-- ─── Validation trigger function ─────────────────────────────────────────

CREATE OR REPLACE FUNCTION validate_goat_cut_sheet()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_goat_cut_sheet_validate ON goat_cut_sheets;
CREATE TRIGGER trg_goat_cut_sheet_validate
    BEFORE INSERT OR UPDATE ON goat_cut_sheets
    FOR EACH ROW EXECUTE FUNCTION validate_goat_cut_sheet();

-- ─── Seed defaults ───────────────────────────────────────────────────────

INSERT INTO goat_cut_sheet_defaults (name, selections) VALUES
('standard', '{
